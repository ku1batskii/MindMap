...
const TITLE_MAX_CHARS=14, NOTE_MAX_CHARS=17;
const PAD_TOP=8, PAD_BOT=7, TITLE_LH=15, DIV_GAP=4, DIV_H=1, NOTE_GAP=5, NOTE_LH=12, BADGE_H=12;

// ── localStorage sessions ─────────────────────────────────────────────────────
const LS_KEY = "mindmap_sessions_v1";
...
function saveSessions(sessions) {
  if (typeof window === "undefined") return;
  try { localStorage.setItem(LS_KEY, JSON.stringify(sessions.slice(0, 20))); } catch {}
}
...
function wrapText(text, maxChars) {
  ...
  for (const w of words) {
    const c=cur?cur+" "+w:w;
    if (c.length<=maxChars){cur=c;}else{if(cur)lines.push(cur);cur=w;}
  }
...
function computeRadialLayout(tree,W,H){
  ...
  orphans.forEach((n,i)=>{
    ...
    const nx=cx+L1R*Math.cos(angle),ny=cy+L1R*Math.sin(angle);
    pos[n.id]={x:nx,y:ny};
...
async function fetchMap(input,tree){
  ...
  const p1="Return ONLY raw JSON, no markdown, no backticks.\n"+
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:300,background:dark?"#000":"#e8f5ec",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",borderBottom:`1px solid ${faint}`,flexShrink:0}}>
        <span style={{fontSize:9,color:dark?"rgba(0,255,136,0.45)":"rgba(0,100,40,0.6)",letterSpacing:4,fontFamily:"'Courier New',monospace"}}>{isHome?"MIND MAP":"INPUT"}</span>
        {isHome?(
          <button onClick={onThemeToggle} style={{width:30,height:30,borderRadius:5,background:"transparent",border:`1px solid ${faint}`,color:dark?"rgba(0,255,136,0.7)":"rgba(0,120,50,0.8)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            {dark?<svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>:<svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>}
          </button>
        ):(
          <button onClick={onClose} style={{background:"transparent",border:`1px solid ${faint}`,color:dark?"rgba(0,255,136,0.7)":"rgba(0,120,50,0.8)",fontFamily:"'Courier New',monospace",fontSize:9,padding:"4px 12px",cursor:"pointer",letterSpacing:2,borderRadius:4}}>✕ CLOSE</button>
        )}
      </div>
...
// ── Sessions sidebar ──────────────────────────────────────────────────────────
function SessionsSidebar({sessions,onSelect,onDelete,onClose,onNew,theme}){
  const dark=theme==="dark";
  const C=dark?{bg:"#0a120a",border:"#1e4428",text:"#00ff88",textDim:"rgba(0,255,136,0.45)",cardBg:"#0f1a0f"}
              :{bg:"#e8f5e8",border:"#4a8a5a",text:"#004d18",textDim:"rgba(0,80,30,0.5)",cardBg:"#d0e8d0"};
  return(
    <>
      {/* backdrop */}
      <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:190,background:"rgba(0,0,0,0.55)"}}/>
      {/* panel */}
      <div style={{position:"fixed",top:0,left:0,bottom:0,width:"78vw",maxWidth:320,zIndex:200,background:C.bg,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",overflowY:"hidden"}}>
        {/* header */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 14px 12px",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"none",color:C.textDim,fontSize:20,cursor:"pointer",lineHeight:1,padding:"0 4px"}}>✕</button>
        </div>
        {/* new session */}
        ...
        <div style={{flex:1,overflowY:"auto",padding:"4px 0"}}>
          {sessions.length===0&&(
            <div style={{padding:"24px 16px",fontSize:11,color:C.textDim,fontFamily:"'Courier New',monospace",textAlign:"center"}}>No saved sessions yet</div>
          )}
          {sessions.map((s,i)=>(
            <div key={s.id} style={{display:"flex",alignItems:"center",borderBottom:`1px solid ${C.border}`,margin:"0 12px"}}>
              <button onClick={()=>onSelect(s)}
                ...
                <div style={{fontSize:12,marginBottom:2}}>{trunc(s.goal||"Untitled",28)}</div>
                <div style={{fontSize:9,color:C.textDim}}>{new Date(s.ts).toLocaleDateString()} · {s.nodes} nodes</div>
              </button>
              <button onClick={()=>onDelete(s.id)}
                style={{background:"none",border:"none",color:C.textDim,fontSize:16,cursor:"pointer",padding:"8px 6px",flexShrink:0}}>
                ×
              </button>
            </div>
          ))}
...
export default function MindMap(){
  ...
  const dark=theme==="dark";
  const C=dark?{bg:"#090909",bgSub:"#060d06",border:"#1e4428",accent:"#00ff88",accentDim:"rgba(0,255,136,0.82)",accentFaint:"rgba(0,255,136,0.2)",text:"#00ff88",textDim:"rgba(0,255,136,0.48)",cardBg:"#0a120a",dots:"#131a13"}
  ...
  useEffect(()=>{
    ...
    setPos(newPos);flushTransform(fitAll(newPos,w,h));
  },[tree,view]);
  ...
  const persistSession=useCallback((savedTree)=>{
    if(!savedTree.goal||!savedTree.nodes.length)return;
    const existing=loadSessions();
    const entry={id:savedTree.goal+"_"+Date.now(),goal:savedTree.goal,nodes:savedTree.nodes.length,ts:Date.now(),tree:savedTree};
    const updated=[entry,...existing.filter(s=>s.goal!==savedTree.goal)].slice(0,20);
  ...
  const getNavOrder=useCallback(()=>{
    ...
    const bfs=ids=>{if(!ids.length)return;const next=[];ids.forEach(id=>{if(visited.has(id))return;visited.add(id);order.push(id);treeRef.current.nodes.filter(c=>c.parentId===id).forEach(c=>next.push(c.id));});bfs(next);};
  ...
  const focusNode=useCallback(id=>{
    ...
    const{width:W,height:H}=svg.getBoundingClientRect();
    const sc=1.55;flushTransform({x:W/2-sc*p.x,y:H/2-sc*p.y,scale:sc});setSelectedId(id);
  },[]);
  ...
  useEffect(()=>{
    ...
    const onWheel=e=>{
      ...
      const rect=svg.getBoundingClientRect(),px=e.clientX-rect.left,py=e.clientY-rect.top;
  ...
  // Pointer + long-press for edit menu
  useEffect(()=>{
    const svg=svgRef.current;if(!svg)return;
    const ptrs=new Map();
    ...
    let lastMid=null,lastDist=null,panStart=null,nodeDrag=null,lpTimer=null,lpNode=null;

    const clearLP=()=>{if(lpTimer){clearTimeout(lpTimer);lpTimer=null;}lpNode=null;};
    ...
    const onDown=e=>{
      ...
      }else if(ptrs.size===2){clearLP();nodeDrag=null;panStart=null;lastMid=mid();lastDist=pdist();}
    };
    const onMove=e=>{
      ...
      if(ptrs.size===1&&nodeDrag){
        ...
        if(Math.sqrt(dx*dx+dy*dy)>6)clearLP();
  ...
  const exportSVG=useCallback(()=>{
    ...
    a.download=(tree.goal||"mindmap").replace(/\s+/g,"_")+".svg";a.click();URL.revokeObjectURL(a.href);
  },[tree.goal]);

  const fetchRelated=useCallback(async()=>{
    if(!tree.goal)return;
    setShowRelated(true);
    if(relatedCache.current[tree.goal]){setRelatedList(relatedCache.current[tree.goal]);return;}
    ...
    try{
      ...
      relatedCache.current[tree.goal]=topics;setRelatedList(topics);
    }catch(e){}
  ...
  const sendModal=()=>{if(!followText.trim()||busy)return;const v=followText;setFollowText("");setShowFollowUp(false);process(v);};

  // Sessions sidebar handlers
  const handleSelectSession=useCallback(s=>{
    setTree(s.tree);setPos({});
    setMapHistory([s.tree]);setHistIdx(0);histIdxRef.current=0;
    setShowSidebar(false);setView("map");
  },[]);
  ...
  const handleNewSession=useCallback(()=>{
    setShowSidebar(false);setView("input");setInputText("");
  },[]);
  ...
  const TOOLBAR_H=44; // consistent height for both toolbar and star btn
  ...
  if(view==="input"){
    return(
      <>
        <InputModal value={inputText} onChange={setInputText}
          onSubmit={()=>{if(!inputText.trim()||busy)return;const v=inputText;setInputText("");setView("map");setTimeout(()=>process(v),80);}}
          onClose={null} busy={busy} isHome={true} theme={theme}
    ...
    <div style={{background:C.bg,color:C.text,fontFamily:"'Courier New',monospace",height:"100dvh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",top:0,left:0,right:0,zIndex:30,display:"flex",alignItems:"flex-start",justifyContent:"space-between",padding:"14px 14px 0",pointerEvents:"none"}}>
        ...
        <div style={{display:"flex",gap:1,background:dark?"rgba(9,9,9,0.88)":"rgba(240,247,240,0.88)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",border:`1px solid ${C.border}`,borderRadius:6,padding:"4px 5px",pointerEvents:"all"}}>
          ...
          <button onClick={saveMap} disabled={!tree.nodes.length} style={{...iconBtn(),color:tree.nodes.length?C.accentDim:C.accentFaint,cursor:tree.nodes.length?"pointer":"default"}}>
      ...
      <div style={{flex:1,overflow:"hidden",position:"relative"}}>
            ...
            {tree.nodes.map(n=>{
              ...
              return(
                <g key={n.id} transform={`translate(${p.x},${p.y})`}
                  ...
                  <text textAnchor="start" x={-NW/2+7} y={nh/2-3} fill={ns.stroke} fontSize={6.5} opacity={0.35} letterSpacing={1} fontFamily="'Courier New',monospace" style={{pointerEvents:"none"}}>{n.type.toUpperCase()}</text>
                  <circle cx={NW/2-9} cy={nh/2-6} r={2.5} fill={n.confidence==="high"?ns.stroke:"none"} stroke={ns.stroke} strokeWidth={1} opacity={0.38} style={{pointerEvents:"none"}}/>
                </g>
              );
            })}
          </g>
        ...
        <div style={{
          ...
          {/* Star / Related — same height as toolbar */}
          <button onClick={fetchRelated} title="Related topics"
            style={{
              width:TOOLBAR_H,height:TOOLBAR_H,borderRadius:8,flexShrink:0,
              background:dark?"rgba(9,9,9,0.88)":"rgba(240,247,240,0.88)",
              backdropFilter:"blur(14px)",WebkitBackdropFilter:"blur(14px)",
              border:`1px solid ${C.border}`,color:C.accentDim,
          ...
          <div style={{
            ...
            <button onClick={fit} title="Fit" style={pillBtn()}>
            ...
            <button onClick={()=>setEditMode(m=>!m)} title="Edit mode" style={pillBtn(false,editMode)}>
      ...
      {showFollowUp&&(
        <InputModal value={followText} onChange={setFollowText}
          onSubmit={sendModal} onClose={()=>setShowFollowUp(false)}
          busy={busy} isHome={false} theme={theme}
          onThemeToggle={()=>setTheme(t=>t==="dark"?"light":"dark")}/>
      )}

      {/* Sessions sidebar */}
      {showSidebar&&(
        <SessionsSidebar sessions={sessions} onSelect={handleSelectSession}
          onDelete={handleDeleteSession} onClose={()=>setShowSidebar(false)}
          onNew={handleNewSession} theme={theme}/>
      )}
