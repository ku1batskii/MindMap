...
const TRENDING = [
  ...
  { icon:"🎨", label:"UI Design Portfolio" },
];
...
function computeRadialLayout(tree, W, H) {
  ...
  orphans.forEach((n, i) => {
    ...
    const L2R = 150;
    const fanSpan = Math.min(Math.PI * 0.62, children.length * 0.36);
    children.forEach((c, j) => {
      ...
      const L3R = 110;
      const gFan = Math.min(Math.PI * 0.32, gcs.length * 0.2);
      gcs.forEach((gc, k) => {
...
async function fetchMap(input, tree) {
  ...
  const p1 =
    ...
    "Rules: Extract 4-10 distinct ideas. note=clear summary 1-2 sentences. type: idea=concept(BLUE), subgoal=goal(GREEN), step=action(GRAY), risk=danger/problem(RED), alternative=other option(PURPLE). risk for ANYTHING negative. " +
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      drops.forEach((y, i) => {
        ...
        ctx.fillStyle = bright ? "rgba(180,255,180,0.95)" : "rgba(0,255,65,0.5)";
...
export default function MindMap() {
  ...
  const C = dark ? {
    bg: "#090909", bgSub: "#060d06", border: "#1e4428", accent: "#00ff88",
    accentDim: "rgba(0,255,136,0.82)", accentFaint: "rgba(0,255,136,0.2)",
    text: "#00ff88", textDim: "rgba(0,255,136,0.48)", cardBg: "#0a120a", dots: "#131a13",
  } : {
    bg: "#f0f7f0", bgSub: "#e0ede0", border: "#4a8a5a", accent: "#006622",
    accentDim: "rgba(0,80,30,0.82)", accentFaint: "rgba(0,80,30,0.2)",
    text: "#004d18", textDim: "rgba(0,80,30,0.48)", cardBg: "#d0e8d0", dots: "#b0ccb0",
  };

  const [tree, setTree] = useState({ goal: "", nodes: [] });
  const [pos, setPos] = useState({});
  const [log, setLog] = useState([{ c: "s", t: "MIND MAP -- введи текст внизу" }, { c: "s", t: "/mock -- тест · /clear -- сброс" }]);
  ...
  const treeRef = useRef(tree), posRef = useRef({}), transformRef = useRef({ x: 0, y: 0, scale: 1 });
  const editModeRef = useRef(false), modalInputRef = useRef(null);
  ...
  const getNavOrder = useCallback(() => {
    ...
    const bfs = ids => {
      ...
      ids.forEach(id => {
        ...
        treeRef.current.nodes.filter(c => c.parentId === id).forEach(c => next.push(c.id));
      });
      bfs(next);
    };
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      if (ptrs.size === 1 && nodeDrag) {
        const d = nodeDrag; setPos(p => ({ ...p, [d.id]: { x: d.ox + (e.clientX - d.sx), y: d.oy + (e.clientY - d.sy) } }));
  ...
  const process = useCallback(async val => {
    ...
    if (val === "/mock") {
      ...
      setTree(mock); setPos({}); setMapHistory([mock]); setHistIdx(0); histIdxRef.current = 0; lg("o", "- mock " + mock.nodes.length + " nodes -"); return;
    }
    if (busy) return;
  ...
  const navBack = useCallback(() => {
    ...
    const ni = histIdxRef.current - 1; setTree(mapHistory[ni]); setPos({}); setHistIdx(ni); histIdxRef.current = ni;
  }, [mapHistory]);
  ...
  const saveMap = useCallback(async () => {
    ...
    try {
      ...
      try { await navigator.clipboard.writeText(window.location.origin + "/view/" + data.slug); } catch (e) {}
  ...
  const startSession = useCallback(val => { setView("map"); setTimeout(() => process(val), 80); }, [process]);
  const sendModal = () => { if (!modalText.trim() || busy) return; const v = modalText; setModalText(""); setShowInputModal(false); process(v); };
  ...
  const pillBtn = (disabled = false, active = false) => ({
    width: 40, height: 40, borderRadius: "50%", background: active ? (dark ? "rgba(0,255,136,0.1)" : "rgba(0,80,30,0.1)") : "none",
    ...
    return (
      <div style={{ background: C.bg, color: C.text, fontFamily: "'Courier New',monospace", height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 14px", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          <span style={{ fontSize: 10, color: C.textDim, letterSpacing: 4 }}>MIND MAP</span>
          <button onClick={() => setTheme(t => t === "dark" ? "light" : "dark")}
            style={{ background: "transparent", border: `1px solid ${C.border}`, color: C.accentDim, fontFamily: "'Courier New',monospace", fontSize: 9, padding: "4px 10px", cursor: "pointer", letterSpacing: 2 }}>
            {dark ? "☀ LIGHT" : "☾ DARK"}
          </button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 16px 32px" }}>
          <div style={{ marginBottom: 28 }}>
            <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: 2, lineHeight: 1.2, marginBottom: 4 }}>MIND MAP</div>
            <div style={{ fontSize: 9, color: C.textDim, letterSpacing: 4 }}>STRUCTURED THINKING ENGINE</div>
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 36 }}>
            <input value={homeInput} onChange={e => setHomeInput(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter" && homeInput.trim()) startSession(homeInput); }}
              placeholder={CLIENT_CONFIG.placeholder} autoFocus
              style={{ flex: 1, background: C.cardBg, border: `1px solid ${C.border}`, color: C.text, fontFamily: "'Courier New',monospace", fontSize: 13, padding: "11px 12px", outline: "none" }} />
            <button onClick={() => { if (homeInput.trim()) startSession(homeInput); }}
              style={{ background: "transparent", border: `1px solid ${C.accentDim}`, color: C.accent, fontFamily: "'Courier New',monospace", fontSize: 20, padding: "0 18px", cursor: "pointer" }}>→</button>
          </div>
          <div style={{ fontSize: 9, color: C.textDim, letterSpacing: 4, marginBottom: 12 }}>🔥 TRENDING</div>
          {TRENDING.map((t, i) => (
            <button key={i} onClick={() => startSession(t.label)}
    ...
    <div style={{ background: C.bg, color: C.text, fontFamily: "'Courier New',monospace", height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 30, display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "14px 14px 0", pointerEvents: "none" }}>
        <button onClick={() => setView("home")}
          style={{ width: 42, height: 42, borderRadius: "50%", background: dark ? "rgba(9,9,9,0.85)" : "rgba(240,247,240,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${C.border}`, color: C.accentDim, fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "all" }}>‹</button>
        <div style={{ display: "flex", gap: 2, background: dark ? "rgba(9,9,9,0.85)" : "rgba(240,247,240,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${C.border}`, borderRadius: 12, padding: "5px 6px", pointerEvents: "all" }}>
          {[
            [<svg key="dl" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" /></svg>, exportSVG, "SVG"],
            [dark ? <svg key="sun" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><circle cx="12" cy="12" r="5" /><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" /></svg> : <svg key="moon" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z" /></svg>, () => setTheme(t => t === "dark" ? "light" : "dark"), "theme"],
            [<svg key="warn" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>, () => setShowReport(true), "report"],
          ].map(([icon, fn, key]) => (
            <button key={key} onClick={fn} style={{ width: 34, height: 34, borderRadius: 8, background: "none", border: "none", cursor: "pointer", color: C.accentDim, display: "flex", alignItems: "center", justifyContent: "center" }}>{icon}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
        <svg ref={svgRef} style={{ width: "100%", height: "100%", position: "absolute", inset: 0, cursor: editMode ? "crosshair" : "grab", touchAction: "none" }}>
          <defs>
            <marker id="arr" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="5" markerHeight="5" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="rgba(0,220,100,0.38)" /></marker>
          ...
          <g ref={gRef} style={{ transform: `translate(${transform.x}px,${transform.y}px) scale(${transform.scale})`, willChange: "transform", transformOrigin: "0 0" }}>
            {edges.map(e => {
              ...
              return <path key={e.id} d={d} fill="none" stroke="rgba(0,220,100,0.28)" strokeWidth="1.2" markerEnd="url(#arr)" />;
            })}
            {tree.goal && pos["ROOT"] && (
              <g transform={`translate(${pos["ROOT"].x},${pos["ROOT"].y})`} data-nodeid="ROOT" style={{ cursor: "grab" }}>
                <rect x={-RW / 2} y={-RH / 2} width={RW} height={RH} rx={10} fill={C.cardBg} stroke={C.accent} strokeWidth={1.5} />
            ...
            {tree.nodes.map(n => {
              ...
              return (
                <g key={n.id} transform={`translate(${p.x},${p.y})`}
                  ...
                  data-nodeid={n.id}>
                  {isSel && <rect x={-NW / 2 - 3} y={TOP - 3} width={NW + 6} height={nh + 6} rx={8} fill="none" stroke={C.accent} strokeWidth={1.5} opacity={0.5} />}
                  <rect x={-NW / 2} y={TOP} width={NW} height={nh} rx={6} fill={fill} stroke={editMode ? "rgba(0,255,136,0.55)" : stroke} strokeWidth={editMode ? 2 : 1.5} strokeDasharray={n.confidence === "low" ? "4 3" : undefined} opacity={n.confidence === "low" ? 0.72 : 1} />
                  {tl.map((line, li) => <text key={"t" + li} x={-NW / 2 + 9} y={TOP + PAD_TOP + TITLE_LH * 0.82 + li * TITLE_LH} fill={color} fontSize={11} fontWeight="700" fontFamily="'Courier New',monospace" style={{ pointerEvents: "none" }}>{line}</text>)}
                  {nl.length > 0 && <line x1={-NW / 2 + 9} y1={TOP + PAD_TOP + tl.length * TITLE_LH + DIV_GAP} x2={NW / 2 - 9} y2={TOP + PAD_TOP + tl.length * TITLE_LH + DIV_GAP} stroke={stroke} strokeWidth={0.5} opacity={0.3} />}
                  {nl.map((line, li) => { const dy = TOP + PAD_TOP + tl.length * TITLE_LH + DIV_GAP + DIV_H + NOTE_GAP + NOTE_LH * 0.82; return <text key={"n" + li} x={-NW / 2 + 9} y={dy + li * NOTE_LH} fill={color} fontSize={9.5} opacity={0.6} fontFamily="'Courier New',monospace" style={{ pointerEvents: "none" }}>{line}</text>; })}
                  <text textAnchor="start" x={-NW / 2 + 7} y={nh / 2 - 3} fill={stroke} fontSize={6.5} opacity={0.35} letterSpacing={1} fontFamily="'Courier New',monospace" style={{ pointerEvents: "none" }}>{n.type.toUpperCase()}</text>
                  <circle cx={NW / 2 - 9} cy={nh / 2 - 6} r={2.5} fill={n.confidence === "high" ? stroke : "none"} stroke={stroke} strokeWidth={1} opacity={0.38} style={{ pointerEvents: "none" }} />
                </g>
              );
            })}
          </g>
        ...
        {editTarget && (
          ...
          {[["DEL", () => { const nd = treeRef.current.nodes.find(n => n.id === editTarget.id); if (nd) { setTree(prev => ({ ...prev, nodes: prev.nodes.filter(n => n.id !== editTarget.id).map(n => n.parentId === editTarget.id ? { ...n, parentId: nd.parentId } : n) })); setPos(prev => { const p = { ...prev }; delete p[editTarget.id]; return p; }); } setEditTarget(null); }], ["NOTE", () => { const nd = treeRef.current.nodes.find(n => n.id === editTarget.id); const v = prompt("Редактировать:", nd?.note || ""); if (v !== null) setTree(prev => ({ ...prev, nodes: prev.nodes.map(n => n.id === editTarget.id ? { ...n, note: v } : n) })); setEditTarget(null); }], ["✕", () => setEditTarget(null)]].map(([lbl, fn]) => (
              <button key={lbl} onClick={fn} style={{ background: C.cardBg, border: `1px solid ${C.border}`, color: C.accentDim, fontFamily: "'Courier New',monospace", fontSize: 10, padding: "3px 8px", cursor: "pointer", letterSpacing: 1, borderRadius: 3 }}>{lbl}</button>
            ))}
          </div>
        )}

        {/* Legend */}
        <div style={{ position: "absolute", top: 66, right: 12, display: "flex", flexDirection: "column", gap: 4 }}>
          {Object.entries(TSTROKE).map(([t, c]) => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 7.5, opacity: 0.55, color: C.text, letterSpacing: 1 }}>
              <div style={{ width: 7, height: 7, borderRadius: 2, background: TFILL[t], border: `1px solid ${c}`, flexShrink: 0 }} />{t.slice(0, 3)}
            </div>
          ))}
        </div>
        <div style={{ position: "absolute", top: 66, left: 14, fontSize: 8, color: C.textDim, letterSpacing: 2 }}>{tree.nodes.length} nodes</div>

        {/* ── Pill toolbar ───────────────────────────────────────────────── */}
        <div style={{
          position: "absolute", bottom: 72, left: "50%", transform: "translateX(-50%)", zIndex: 20,
          display: "flex", alignItems: "center", gap: 0,
          background: dark ? "rgba(9,9,9,0.88)" : "rgba(240,247,240,0.88)",
          backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)",
          border: `1px solid ${C.border}`, borderRadius: 50, padding: "5px 8px",
        }}>
          {/* Fit */}
          ...
          {/* Nav prev */}
          <button onClick={() => navNode(-1)} title="Prev node" style={pillBtn(!tree.nodes.length)}>
            <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7" /></svg>
          </button>
          {/* Nav next */}
          <button onClick={() => navNode(1)} title="Next node" style={pillBtn(!tree.nodes.length)}>
        ...
        <button onClick={fetchRelated}
          style={{ position: "absolute", bottom: 76, left: 14, background: dark ? "rgba(9,9,9,0.85)" : "rgba(240,247,240,0.85)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${C.border}`, color: C.accentDim, fontFamily: "'Courier New',monospace", fontSize: 8, padding: "6px 10px", cursor: "pointer", letterSpacing: 2, borderRadius: 20 }}>
          RELATED →
        </button>

        {busy && (
          <div style={{ position: "absolute", inset: 0, background: dark ? "rgba(9,9,9,0.48)" : "rgba(240,247,240,0.48)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px calc(8px + env(safe-area-inset-bottom)) 14px", borderTop: `1px solid ${C.border}`, background: C.bgSub, flexShrink: 0 }}>
        ...
        {/* Tap to open modal */}
        <button onClick={() => { setModalText(""); setShowInputModal(true); setTimeout(() => modalInputRef.current?.focus(), 100); }}
          style={{ flex: 1, background: C.cardBg, border: `1px solid ${C.border}`, color: C.textDim, fontFamily: "'Courier New',monospace", fontSize: 13, padding: "10px 12px", cursor: "text", textAlign: "left", outline: "none" }}>
          {busy ? "…" : "ask follow-up…"}
        </button>
        <button onClick={() => { setModalText(""); setShowInputModal(true); setTimeout(() => modalInputRef.current?.focus(), 100); }}
          style={{ width: 36, height: 36, borderRadius: "50%", background: "transparent", border: `1px solid ${C.accentDim}`, color: C.accentDim, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
          <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
        </button>
      </div>

      {/* ══ FULL-SCREEN INPUT MODAL + MATRIX ════════════════════════════════ */}
      {showInputModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 300, background: "#000", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <MatrixRain />
          <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderBottom: "1px solid rgba(0,255,136,0.15)", flexShrink: 0 }}>
            <span style={{ fontSize: 9, color: "rgba(0,255,136,0.45)", letterSpacing: 4 }}>INPUT</span>
            <button onClick={() => setShowInputModal(false)} style={{ background: "transparent", border: "1px solid rgba(0,255,136,0.3)", color: "rgba(0,255,136,0.7)", fontFamily: "'Courier New',monospace", fontSize: 9, padding: "4px 12px", cursor: "pointer", letterSpacing: 2 }}>✕ CLOSE</button>
          </div>
          <div style={{ position: "relative", zIndex: 1, flex: 1, display: "flex", flexDirection: "column", padding: "20px 16px 16px", gap: 14 }}>
            <textarea ref={modalInputRef} value={modalText} onChange={e => setModalText(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey && !e.metaKey) { e.preventDefault(); sendModal(); } }}
              placeholder={CLIENT_CONFIG.placeholder} rows={10}
              style={{ flex: 1, background: "rgba(0,0,0,0.72)", border: "1px solid rgba(0,255,136,0.22)", color: "#00ff88", fontFamily: "'Courier New',monospace", fontSize: 15, lineHeight: 1.65, padding: "16px", outline: "none", resize: "none", caretColor: "#00ff88", borderRadius: 6 }} />
            <button onClick={sendModal} disabled={busy || !modalText.trim()}
              style={{ background: "transparent", border: `1px solid ${busy || !modalText.trim() ? "rgba(0,255,136,0.18)" : "rgba(0,255,136,0.75)"}`, color: busy || !modalText.trim() ? "rgba(0,255,136,0.22)" : "#00ff88", fontFamily: "'Courier New',monospace", fontSize: 11, padding: "14px", cursor: busy || !modalText.trim() ? "default" : "pointer", letterSpacing: 3, borderRadius: 6 }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "flex-end" }} onClick={e => { if (e.target === e.currentTarget) setShowReport(false); }}>
          <div style={{ width: "100%", background: C.bg, border: `1px solid ${C.border}`, borderRadius: "14px 14px 0 0", padding: "24px 16px calc(28px + env(safe-area-inset-bottom))" }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <button onClick={() => setShowReport(false)} style={{ flex: 1, background: "transparent", border: `1px solid ${C.border}`, color: C.accentDim, fontFamily: "'Courier New',monospace", fontSize: 10, padding: "11px 0", cursor: "pointer", letterSpacing: 2 }}>CANCEL</button>
              <button onClick={() => { setShowReport(false); setReportText(""); lg("o", "✓ report sent"); }} style={{ flex: 1, background: "transparent", border: `1px solid ${C.accent}`, color: C.accent, fontFamily: "'Courier New',monospace", fontSize: 10, padding: "11px 0", cursor: "pointer", letterSpacing: 2 }}>SUBMIT</button>
            </div>
          </div>
        </div>
      )}
