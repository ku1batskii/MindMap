...
// Step 2 — generate thematic titles for new nodes
...
“Rules:\n” +
“- Title = thematic label like a book chapter or newspaper headline.\n” +
“- NEVER copy the first words of the note.\n” +
“- Name the TOPIC, not the content.\n” +
  ...
  if (pointers.size === 1) {
    ...
    while (el && el !== svg) {
      if (el.dataset && el.dataset.nodeid) {
        const p = posRef.current[el.dataset.nodeid] || { x: 0, y: 0 };
        nodeDrag = { id: el.dataset.nodeid, ox: p.x, oy: p.y, sx: e.clientX, sy: e.clientY };
        return;
      }
      el = el.parentElement;
    }
    ...
    lastMid = midpoint(); lastDist = pinchDist();
  }
};
...
const onMove = e => {
  ...
  if (pointers.size === 1 && nodeDrag) {
    const drag = nodeDrag;
    setPos(p => ({ ...p, [drag.id]: { x: drag.ox + e.clientX - drag.sx, y: drag.oy + e.clientY - drag.sy } }));
...
{ id: “n2”, title: “Быстрый кэшфлоу”,   note: “Клиентские проекты генерируют немедленный доход и фундамент для реинвестиций”, type: “step”,        confidence: “high”,   parentId: “n1” },
{ id: “n3”, title: “Подписная модель”,   note: “ContentOS с MRR — платформа растёт без линейного роста затрат”,               type: “step”,        confidence: “high”,   parentId: “n1” },
{ id: “n4”, title: “Пассивный доход”,    note: “Шаблоны и курсы продаются через Gumroad без участия в процессе”,             type: “step”,        confidence: “medium”, parentId: “n1” },
{ id: “n5”, title: “Автоматизация”,      note: “Публикация в Instagram через API Meta для роста аудитории”,                  type: “idea”,        confidence: “medium”, parentId: “n3” },
{ id: “n6”, title: “Платформенный риск”, note: “Meta может ограничить API или снизить охват автоматизированного постинга”,    type: “risk”,        confidence: “high”,   parentId: “n5” },
{ id: “n7”, title: “Проверка спроса”,    note: “Лендинг и waitlist до написания кода для валидации идеи”,                    type: “step”,        confidence: “high”,   parentId: “n3” },
{ id: “n8”, title: “Альтернативный путь”, note: “UI-киты на Gumroad как запасной вариант при медленном росте SaaS”,           type: “alternative”, confidence: “medium”, parentId: “n4” }
]
};
setTree(mock); setPos({});
...
const fresh = updated.nodes.filter(n => !ids.has(n.id));
    ...
    <svg ref={svgRef} style={{ width: "100%", height: "100%", position: "absolute", inset: 0, cursor: "grab", touchAction: "none" }}>
      ...
      <g ref={gRef} style={{ transform: `translate(${transform.x}px,${transform.y}px) scale(${transform.scale})`, willChange: "transform", transformOrigin: "0 0" }}>
        ...
        {tree.nodes.map(n => {
          ...
          const TOP  = -nh / 2;
          const curY = TOP + PAD_TOP + TITLE_LH * 0.82;

          return (
            <g key={n.id} transform={`translate(${p.x},${p.y})`} style={{ cursor: "grab" }} data-nodeid={n.id}>
              <rect x={-NW/2} y={TOP} width={NW} height={nh} rx={6}
                fill={fill} stroke={stroke} strokeWidth={1.5}
                strokeDasharray={dash} opacity={n.confidence === "low" ? 0.72 : 1} />

              {titleLines.map((line, li) => (
                <text key={"t"+li} x={-NW/2+10} y={curY + li*TITLE_LH}
              ...
              {noteLines.map((line, li) => {
                ...
                return (
                  <text key={"n"+li} x={-NW/2+10} y={noteY0 + li*NOTE_LH}
